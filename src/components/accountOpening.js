...
const AccountOpening = () => {
  ...
  useEffect(() => {
    ...
    const fetchProfilePhoto = async (customerId) => {
        try {
          const response = await axios.get('https://firestore.googleapis.com/v1/projects/bankmanagement-136e1/databases/(default)/documents/documents');
      ...
      const fetchCustomers = async () => {
        try {
          const response = await axios.get('https://firestore.googleapis.com/v1/projects/bankmanagement-136e1/databases/(default)/documents/customer');
  ...
  const handleApprove = async (customerId) => {
    ...
    try {
        ...
        const response = await fetch(`https://firestore.googleapis.com/v1/projects/bankmanagement-136e1/databases/(default)/documents/customer/${documentId}?${updateFields}`, {
    ...
    try {
        ...
        const response2 = await axios.post(
          ...
          for(let account=0;account<response2.data.length;account++){
            ...
            let updateFields = `updateMask.fieldPaths=isDelete`;
            const response = await fetch(`https://firestore.googleapis.com/v1/projects/bankmanagement-136e1/databases/(default)/documents/account/${documentId}?${updateFields}`, {
                method: 'PATCH',  // Use PATCH for partial updates
                headers: {
                    'Content-Type': 'application/json'
                },
